...
from backend.models.property import Property # Importa o modelo de Imóvel
...
class DataProcessorService:
    ...
    def process_and_save_properties(self, properties_data: list[dict]):
        """
        Processa uma lista de dicionários de imóveis e os salva no banco de dados.
        Verifica se um imóvel com a mesma URL já existe antes de criar um novo.
        """
        if not properties_data:
            logging.info("Nenhum dado de imóvel para processar.")
            return

        # Primeiro processa os dados para limpá-los
        processed_properties = self.processor.process_properties_list(properties_data)
        
        if not processed_properties:
            logging.warning("Nenhuma propriedade válida após processamento.")
            return

        logging.info(f"Processando {len(processed_properties)} imóveis válidos para salvar no banco de dados...")
        
        new_properties_count = 0
        existing_urls = {prop.url for prop in self.db.query(Property.url).all()}

        for prop_data in processed_properties:
            # Pula se não houver URL ou se a URL já existir no banco
            if not prop_data.get('url') or prop_data['url'] in existing_urls:
                continue

            # Cria uma nova instância do modelo Property com os dados processados
            new_property = Property(
                title=prop_data.get('property_type'),  # Usa o tipo como título
                price=prop_data.get('price'),
                address=prop_data.get('raw_address'),  # Endereço bruto para preservar informação
            ...
            existing_urls.add(new_property.url) # Adiciona à verificação local para eficiência
            new_properties_count += 1
        
        try:
            self.db.commit()
            logging.info(f"Sucesso! {new_properties_count} novos imóveis foram salvos no banco de dados.")
        except Exception as e:
            self.db.rollback()
...
class DataProcessorService:
    ...
    def process_and_save_properties(self, properties_data: list[dict]):
        ...
        logging.info(f"Processando {len(properties_data)} imóveis para salvar no banco de dados...")
        ...
        for prop_data in properties_data:
            ...
            # Cria uma nova instância do modelo Property com os dados do scraper
            new_property = Property(
        ...
        try:
            self.db.commit()
            logging.info(f"{new_properties_count} novos imóveis foram salvos no banco de dados.")
        except Exception as e:
            logging.error(f"Erro ao comitar a sessão no banco de dados: {e}", exc_info=True)
            self.db.rollback() # Desfaz as alterações em caso de erro